...
Some of the analyses in this tutorial can be time consuming, and hence the data set will be reduced to six FOVs:

```{r filter_fovs_chunk}
fovs_keep <- c('Lung5_Rep1_fov_11', 'Lung5_Rep1_fov_6', 'Lung5_Rep1_fov_2', 'Lung6_fov_4', 'Lung6_fov_6', 'Lung6_fov_7')
fovs_rm <- names(lung@counts)[!(names(lung@counts) %in% fovs_keep)]

lung <- filter_data(lung, rm_tissue=fovs_rm)
```
...
```{r stclust_smi}
lung <- STclust(lung, ws=0.02, ks='dtc')
```
...
dom_p <- STplot(lung, ks='dtc', ws=0.02, deepSplit=F,
...
```{r domain_de}
deg <- STdiff(lung, w=0.02, k='dtc', deepSplit=F, test_type='wilcoxon')
```
