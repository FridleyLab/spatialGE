...
```{r toral_reads, fig.height=1.5, fig.width=4.5}
qcplots1 = plot_QC_stats(prvisium, qc='total_reads')
qcplots1
```

```{r total_genes, fig.height=1.5, fig.width=4.5}
...
we use `ggarrange()`:
```{r quilt_ngenes, fig.width=3.5, fig.height=3.5}
qcplots3 = plot_QC_quilt(prvisium, nreads=T)
ggpubr::ggarrange(plotlist=qcplots3, ncol=2, nrow=2)
```

We can filter out spots with low counts (<200). We also see a few spots with
unussually high count in the acinar cell cancer sample. We can filter spots using
...
The `plot_gene_quilt` returns a list of plots.
```{r}
