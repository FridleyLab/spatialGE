...
```{r plot_gene_quilt}
qplot <- plot_gene_quilt(prvisium, genes='FEZF2', color_pal='sunset')
qplot
```
...
Spatial Deconvolution with ESTIMATE is a method determine tumor purity scores. Using ESTIMATE, we can see which spots are more likely to be tumor and which spots are more likely to be stromal.
...
ggpubr::ggarrange(plotlist=eplot, ncol=3, nrow=2, common.legend=T, legend='bottom')
```
