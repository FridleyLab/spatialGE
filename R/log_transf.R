...
#
#
log_transf = function(x=NULL, scale_f=10000, cores=1){
  ...
  x@log_counts = parallel::mclapply(seq_along(x@counts), function(i){
    ...
    # Calculate (spot) library sizes
    # Then, add 1 to each library size.
    libsizes = colSums(x@counts[[i]][, -1])
    #libsizes = libsizes + 1

    # Add 0.5 to all counts (e.g. x + 0.5)
    # Then  multiply for 10^6
    #df = x@counts[[i]][, -1] + 0.5
    df = x@counts[[i]][, -1]
    #df = df * scale_f
