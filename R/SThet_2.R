...
#' @export
#'
SThet_2 = function(x=NULL, genes=NULL, samples=NULL, method='moran', overwrite=F){
  ...
  if('moran' %in% method){
    x = gene_moran_I2(x=x, combo=combo, overwrite=overwrite)
  }
  if('geary' %in% method){
    x = gene_geary_C2(x=x, combo=combo, overwrite=overwrite)
  }
...
#
#
gene_geary_C2 = function(x=NULL, combo=NULL, overwrite=overwrite){
  # Define cores available
  cores = count_cores(nrow(combo))
