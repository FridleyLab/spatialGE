...
# @param genes, a vector with gene names in the count matrix.
# @param subj, the index of the spatial array within the STList.
  ...
  if(is_empty(x@voom_counts[[subj]])){
    ...
    for(gene in genes){
      ...
      if(!any(x@voom_counts[[subj]][[1]] == gene)){
        ...
        x@gene_het[[gene]][[subj]]$gearys_C <- geary_est
