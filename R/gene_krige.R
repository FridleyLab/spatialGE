...
gene_krige <- function(x=NULL, plot_who='top', univ=F, res=0.1){

  # If plot_who='top', get names of 10 genes with the highest standard deviation.
  if(length(plot_who) == 1){
    if(plot_who == 'top'){
      genes <- x@gene_stdev$gene[order(x@gene_stdev$gene_stdevs, decreasing=T)][1:10]
    }else{
      genes <- plot_who
    }
  }else{
  ...
  for(gene in genes){
    ...
    gene_expr <- x@voom_counts[x@voom_counts[[1]] == gene, -1]
    ...
    gene_expr <- gene_expr[match(x@coords[[1]], gene_expr[[1]]), ]
    gene_geo_df <- cbind(x@coords[2:3], as.numeric(gene_expr[[2]]))
    ...
    if(is.null(x@gene_krige[[gene]])){
      x@gene_krige[[gene]] <- list(ord=NULL,
                                   univ=NULL)
    }
    ...
    if(univ == F){
      ...
      x@gene_krige[[gene]]$ord <- gene_krig
      ...
      x@gene_krige[[gene]]$univ <- gene_krig
    }

    # Calculate spatial heterogeneity statistics.
    x <- gene_moran_I(x, genes=gene)
    x <- gene_geary_C(x, genes=gene)
    x <- gene_getis_Gi(x, genes=gene)
